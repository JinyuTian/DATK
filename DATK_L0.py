...
import datetime
import torchattacks
from SelectModel import SelectModel_L0
...
parser.add_argument('--net_type', type=str, help='vgg19 | resnet50 | inception_v3 | densenet121'
...
parser.set_defaults(net_type='vgg19')
...
# parser.set_defaults(data_type='vgg19')
parser.set_defaults(tensorboard=False)
parser.set_defaults(ImageSize=244)
parser.set_defaults(lr=0.5)
...
def main():
    ...
    if args.data_type == 'imagenet':
        nets = ['vgg19','mobilenet_v2','resnet50']
        magnitudes = [6/ 255.0, 7 / 255.0, 8 / 255.0, 10 / 255]

    elif args.data_type == 'cifar10':
        nets = ['densenet121', 'mobilenet_v2', 'resnet50']
        magnitudes = [20/255.0,24/255.0,26/255.0,30/255]
...
def main2(args,Exp_log):
    ...
    for N_count,(image,label) in enumerate(ValDataloader):
        ...
        if not torch.argmax(output) == target:
            ...
            log = mode + 'succ_atk/succ_pred/n_sample: [{}]/[{}]/[{}] Net_type: {}, Adv_type: {}, ASR ({:.2f})! Magnitude (L_inf,L_2,L_0) [{L_inf:.4f},{L_2:.4f},{L_0:.4f}]) ### Q_ERROR {q_error}'.format(
                str(succ_count),str(count),str(N_count),args.net_type,args.adv_type,ASR,L_inf = magnitude.L_inf_T, L_2 = magnitude.L_2_T,
                L_0 = magnitude.L_0_T,q_error=q_error)
    ...
    log = mode + 'Net_type: {}, Adv_type: {}, ASR ({:.2f})! Magnitude (L_inf,L_2,L_0) [{L_inf:.4f},{L_2:.4f}(avgl2:.4f),{L_0:.4f}])'.format(
        args.net_type, args.adv_type, ASR, L_inf=magnitude.L_inf,L_2=magnitude.L_2,L_0=magnitude.L_0,avgl2=avgl2)
    print(log)
...
def Attack(args,model,clean,target):
    ...
    adv, Flag = DiscreteAdv(clean,target,model,args,targeted=-1)
...
class get_magnitude():
    ...
    def update(self,adv_data,data):
        ...
        self.L_0_T = np.sum(np.abs(adv_data - data)>0)/adv_data.size
        self.L_0_T = float(self.L_0_T)
        self.L_inf += self.L_inf_T
        self.L_2  += self.L_2_T
        self.L_0 += self.L_0_T

def DiscreteAdv(img,label,model,args,targeted=-1):
    SelectNet = SelectModel_L0(model, ImageScale=args.ImageSize,maxiter=maxiter,magnitude=args.magnitude)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with torch.no_grad():
            ...
            adv_data =  torch.clip(torch.round(adv_input*255.0)/255.0,0.0,255.0)
        ...
        q_error = torch.mean(torch.abs(torch.round(adv_data * 255.0) - adv_data * 255.0))
        log = mode + 'Epoch: [{0}] Loss {loss:.4f} ({Tloss:.4f}) Flag {top1:.3f} L_inf {linf:.4f} L_2 {l2:.4f} L_0 {l1:.4f} ### Q_ERROR {q_error}'.format(
                    epoch,loss=float(loss.data.cpu().numpy()),Tloss=float(total_loss.data/(epoch+1)),linf=magnitude.L_inf_T, l2=magnitude.L_2_T,l1=magnitude.L_0_T,
                    top1=prec1,q_error=q_error)
        import sys
        sys.stdout.write(log)
        # print(log)
        if epoch%20 == 0:
        ...
        for index,flag in enumerate(adv_succ_index):
            if flag:
                adv_noise_succ = (adv_data[index]-imgs[index].cpu())*255.0
                adv_noise_l0 = torch.nonzero(adv_noise_succ).shape[0]/adv_noise_succ.view([1,-1]).shape[1]
                if adv_noise_l0<best_l0:
                    best_adv_data = adv_data[index]
                    best_l0 = adv_noise_l0
        if  best_l0 < 1:
